...
commit snapshot itself as /.porta-winux-commit.json, so it needs no side
...
COMMIT_META = ".porta-winux-commit.json"
...
def checkout(
    ...
    if not got:
        raise PortaWinuxError(
            "checkout matched no files; check the path spelling against 'porta-winux ls'"
        )
    return got
...
def commit(
    ...
    if not files:
        raise PortaWinuxError("workspace is empty; nothing to commit")
    meta = {
        "message": message,
...
def read_commit_meta(store: SnapshotStore, snap: Snapshot) -> dict:
    """Read /.porta-winux-commit.json out of a commit snapshot.
...
def _snapshot_prefix(snap: Snapshot) -> str:
    if not snap.paths:
        raise PortaWinuxError(f"commit snapshot {snap.short_id} records no paths")
