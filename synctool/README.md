...
| synctool term      | what it is                                                            |
|--------------------|-----------------------------------------------------------------------|
| **system snapshot**| restic snapshot of a *profile*'s paths taken on a machine (`snapshot`)|
| **profile**        | named path set + excludes in `synctool.toml` on the drive             |
...
synctool init-drive /run/media/$USER/MYDRIVE
$EDITOR /run/media/$USER/MYDRIVE/synctool.toml   # define profiles
synctool snapshot                 # back up the default profile
```

The drive is auto-detected under `/run/media/$USER`, `/media`, `/mnt`
(whichever mount contains a `synctool.toml`); override with `--drive` or
`SYNCTOOL_DRIVE`.
...
and the source copy in `drive/synctool-src/` if present.

```bash
# browse / edit on any PC
synctool browse                       # Textual TUI: fuzzy filter, preview, Enter=edit
synctool ls latest | grep bashrc      # or plain unix
synctool mount /tmp/snap              # or FUSE-mount everything, use any tool
synctool checkout /home/me/.bashrc    # stage into workspace
$EDITOR <drive>/workspace/root/home/me/.bashrc
synctool commit -m "tweak aliases"

# back on the original machine
synctool sync                         # applies commits; conflicts are flagged, never clobbered
synctool sync --force                 # ...unless you say so
synctool list                         # history
synctool revert <snap-id> [paths…]    # time-travel (auto safety snapshot first)

# fresh Fedora install
synctool restore-full                 # lay down the latest system snapshot
synctool verify                       # restic integrity check
synctool prune --keep-last 10         # thin old system/safety snapshots; commits kept
```
...
writes, so recovery from a bad sync is `synctool revert <safety-id>`.
...
1. **hooks.d/** — drop an executable in
   ...
   Context arrives as `SYNCTOOL_*` env vars (see `src/synctool/hooks.py`).
...
2. **profiles** in `synctool.toml` — new machines, OS slices, or a future
   Windows path set are just more profiles.
3. **`SnapshotStore`** (`src/synctool/store.py`) — the only file that knows
...
```bash
make smoke            # seconds: full e2e cycle in a sandbox (needs restic + python3)
...
│   ├── manifest.py    drive discovery, synctool.toml profiles
...
├── drive-template/    copied onto new drives by init-drive
├── test/
...
- `init-drive` writes the repo password to `.restic-pass` on the drive so a
  ...
  `drive-template/README.md` for the alternative.
...
- Sync/restore write with the invoking user's privileges; system paths need
  `sudo synctool …` (state then lives under root's `~/.local/state`).
