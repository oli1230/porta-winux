#!/usr/bin/env bash
# VM test harness for porta-winux using libvirt + Fedora Cloud + cloud-init.
#
# The "external hard drive" is simulated as a second qcow2 disk (vdb)
# attached to the VM, built from the packaged drive template.
#
...
WORK="${PORTA_WINUX_VM_DIR:-$HERE/.work}"
VM_NAME="${PORTA_WINUX_VM_NAME:-porta-winux-test}"
...
do_push() {
  ...
  rsync -a --delete -e "ssh -i $SSH_KEY -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null" \
    ...
    "$REPO_ROOT/" "test@$(vm_ip):~/porta-winux/"
}

do_test() {
  do_push
  ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null \
    "test@$(vm_ip)" 'sudo bash ~/porta-winux/test/vm/in-vm-test.sh'
}
